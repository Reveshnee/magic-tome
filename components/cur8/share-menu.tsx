...
export default function ShareMenu({
  ...
  function toggle() {
    ...
    if (r) {
      // Anchor the menu above the button, right-aligned to it.
      setAnchor({ x: Math.min(r.right, window.innerWidth - 12), y: r.top })
    }
    setOpen(true)
  }
  ...
  const menu = open && anchor
    ? createPortal(
        <>
          {/* Transparent backdrop — React onClick (not a native document
              listener) so it reliably closes without breaking other handlers. */}
          <div
